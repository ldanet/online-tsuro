  ...
  return (
    <>
      ...
      <g filter="url(#player-shadow)">
        ...
        {Object.entries(colorCoordinates).map(([color, coords]) => {
          if (!coords || coords.isOut) return null;

          const collision = collisions.find((c) =>
            c.colors.includes(color as PlayerColor)
          );
      ...
      <g>
        {collisions.map((c) => {
          ...
          return (
            ...
            >
              <path
                className={cn(styles.splotch, styles[`player_${c.colors[1]}`])}
              ...
              />
              <path
                className={cn(styles.splotch, styles[`player_${c.colors[0]}`])}
